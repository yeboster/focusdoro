...
- Storage: `SessionStore.swift` (Core Data, programmatic `NSManagedObjectModel`), `UserDefaultsPreferencesStore`/`UserDefaultsTimerStateStore` (preferences and in-flight timer state), `KeychainStore.swift` (Todoist token, Keychain only).
...
- `AppPreferences` decodes with the synthesized `Codable`, which calls `decodeIfPresent` for every `Optional` stored property. New preference fields (`taskSortOrderID`, `taskFilterCriteria`, `logsAbandonedTimeFlag`) are declared optional specifically so preferences JSON written by an older build keeps decoding after a new field is added.
