...
- `Sources/FocusdoroCore/Services/FocusPresence.swift` + `MacFocusChannel.swift` / `SlackClient.swift` / `SlackPresenceChannel.swift` — focus mode. A starting session is fanned out by `PresenceCoordinator` (actor) to every `PresenceChannel`: macOS Focus via `/usr/bin/shortcuts run <name>` (no public API exists for setting a Focus) and Slack via `dnd.setSnooze` + `users.profile.set`. Channel failures become one warning banner and never touch the timer. See `docs/specs/focus-mode.md`.
...
- Storage: `SessionStore.swift` (Core Data, programmatic `NSManagedObjectModel`), `UserDefaultsPreferencesStore`/`UserDefaultsTimerStateStore` (preferences and in-flight timer state), `KeychainStore.swift` (Todoist token, plus a separate `<bundle id>.slack` entry for the Slack user token — Keychain only).
...
- The Slack user token follows the same rule as the Todoist one: Keychain only, never echoed into an error. `SlackError` messages are fixed strings, and `SlackClientTests` asserts no failure path contains the token.
